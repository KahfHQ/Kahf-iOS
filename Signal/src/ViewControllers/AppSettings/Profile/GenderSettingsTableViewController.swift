...
class GenderSettingsTableViewController: OWSTableViewController2 {
    ...
    func appearanceItem(_ mode: Gender) -> OWSTableItem {
        return OWSTableItem(
            ...
            actionBlock: { [weak self] in
                self?.normalizedFamilyName = GenderSettingsTableViewController.nameForSystem(mode)
                self?.changeTheme(mode)
                self?.genderSettingsDelegate?.genderDidChange(gender: self?.normalizedFamilyName ?? NSLocalizedString("PROFILE_GENDER_MALE_TEXT",comment: "Gender male text"))
            },
            accessoryType: normalizedFamilyName == GenderSettingsTableViewController.nameForSystem(mode) ? .checkmark : .none
        )
    }
    ...
    static func nameForTheme(_ mode: Gender) -> String {
        ...
        case .male:
            return NSLocalizedString("PROFILE_GENDER_MALE_TEXT",comment: "Gender male text")
        }
    }
 
    static func nameForSystem(_ mode: Gender) -> String {
        switch mode {
        case .female:
            return "Female"
        case .male:
